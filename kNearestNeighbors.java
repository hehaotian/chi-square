...
public class kNearestNeighbors {
   ...
   public void prediction(String test_path, int k_val, boolean isEuclidean, PrintStream sys) throws IOException {
      ...
      while ((testLine = test_br.readLine()) != null) {
         ...
         while ((trainLine = train_br.readLine()) != null) {
            ...
            // if Euclidean distance is chosen, calls get_enclidean function
            // whereas, if Cosine Similarity is chosen, calls get_cosine function
            if (isEuclidean) {
            ...
            if (dist_tally.size() < k_val) {
               ...
               if (isEuclidean) {
                  dist_tally = pick_euc_nearest(dist_tally, new_key, dist);
               } else {
                  dist_tally = pick_cos_nearest(dist_tally, new_key, dist);
               }
            }
            // DEBUG:
   ...
   private Map<String, Double> pick_euc_nearest(Map<String, Double> dist_tally, String new_key, double new_dist) {      
   ...
   private Map<String, Double> pick_cos_nearest(Map<String, Double> dist_tally, String new_key, double new_dist) {      
      if (dist_tally.size() > 1) {
         double min = Double.MAX_VALUE;
         String min_key = "";
         for (String key : dist_tally.keySet()) {
            if (dist_tally.get(key) < min) {
               min = dist_tally.get(key);
               min_key = key;
            }
         }
         if (min < new_dist) {
            dist_tally.put(new_key, new_dist);
            dist_tally.remove(min_key);
         }
         return dist_tally;
      } else {
         Map<String, Double> one_nearest = new HashMap<String, Double>();
   ...
   private double get_euclidean(Map<String, Integer> test, Map<String, Integer> train) {
      double sum = 0.0;
      // traverse all the features in the test document
      for (String feature : test.keySet()) {
         // if this feature exists in both the documents, do the substraction of this feature in both;
         // if this feature doesn't exist in the train document, treat the value in the position as 0;
         // add up all the squared substraction to the sum
      ...
      // traverse all the features in the train document
      for (String feature : train.keySet()) {
         // for only those features not appearing in the test documents
         // treat the feature in the test document as 0;
         // add up all the squared substraction to the sum
      ...
      // return the sum
      // although it should return the square root of the sum
      // but the sum also has the same function for the comparison
      return sum;
   }
   
   private double get_cosine(Map<String, Integer> test, Map<String, Integer> train) {
      double prod = 0.0; // the numerator
      double sq1 = 0.0; // the first part of the denominator
      double sq2 = 0.0; // the second part of the denominator
      // traverse all the test features in the test document
      for (String feature : test.keySet()) {
         // if this feature exists in both the documents, do the substraction of this feature in both;
         // else this numberator should be 0;
         if (train.containsKey(feature)) {
            prod += test.get(feature) * train.get(feature);
         }
         // add up all the squared test feature values to the first part of the denominator
         sq1 += test.get(feature) * test.get(feature);
      }
      // traverse all the train features in the train document
      for (String feature : train.keySet()) {
         // add up all the squared train feature values to the second part of the denominator
