...
## created on 1/29/2013. Modified from 194/e/build_kNN.pl
...
sub read_instances {
	...
	my $res = line_to_inst($line, $valid_cnt,
			       \@inst, $class2idx, $classnames_ptr,
...
### return suc
### the line has the format "label f1:v1 f2:v2 ..."
###
sub line_to_inst {
    my ($line, $inst_id, $inst, $class2idx, $classnames_ptr, 
	$feat2idx, $featnames_ptr, $add_new) = @_;
    ...
    ### push instanceName
    my $instName = "array:$inst_id";
    push(@$inst, $instName);
    
    ### push classLabel
    my $class_label = $parts[0];
    ...
    #### deal with features
    for(my $i=1; $i<$part_num; $i++){
	my @subparts = split(/:/, $parts[$i]);
	if(scalar @subparts != 2){
	    return 0;
	}
			     
	my $featname = $subparts[0];
	my $featval = $subparts[1];
